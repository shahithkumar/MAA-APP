...
class ArtTherapyAnalysisView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Convert to base64 for Groq Vision if it's a file
            if image_file:
                import base64
                image_data = base64.b64encode(image_file.read()).decode('utf-8')
            else:
                # Assuming frontend sends raw base64 or data URI
                if 'base64,' in image_base64:
                    image_data = image_base64.split('base64,')[1]
                else:
                    image_data = image_base64

            # --- AI ANALYSIS USING GROQ VISION ---
            from langchain_groq import ChatGroq
            ...
            prompt_text = """
            You are a warm and insightful Art Therapist. 
            Analyze this mandala/drawing colored by the user. 
            
            Consider:
            1. The specific colors used and what they might symbolize (e.g., cool blues for calm, vibrant reds for energy).
            2. The balance of the composition.
            3. The overall "vibe" or emotional resonance of the piece.

            Provide an empathetic, supportive, and gentle interpretation (4-5 sentences) speaking directly to the user. 
            Avoid clinical jargon. Be encouraging and focus on the therapeutic value of their creative expression.
            """
            ...
            print(f"Art Therapy AI Error: {e}")
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
